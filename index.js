 ...
 * 
 * 共享常量与工具函数的权威文档定义在 ./shared.js 中，本文件内联了一份副本。
 * 修改常量时请同步更新 shared.js 和 taskbar-lyric-window.js 两处。
 */
 ...
 * 窗口的实际位置由浮窗端根据任务栏检测自动计算，主入口不需要处理定位。
 */
 ...
 * @param {Object} values - 新设置值
 * @param {Object} options - 选项（broadcast: 是否广播）
 */
  ...
  ctx.vue.defineComponent({
    ...
    setup(props) {
      ...
      return () => {
        ...
        // 右侧布局由 CSS cover-right 的 flex-direction: row-reverse 翻转
...
const createSettingsComponent = (ctx) =>
  ctx.vue.defineComponent({
    ...
    setup() {
      ...
      // 组件挂载时从 storage 重新加载，确保 draft 包含浮窗端所做的更改
      // 浮窗在 lock 时会将 manualAdjust/偏移写入 storage，但 state.settings 不会自动同步
      onMounted(async () => {
        try {
      ...
      return () =>
        h("div", { class: "tb-lyric-settings" }, [
          ...
          renderSection("歌词内容", "控制歌词和副文本的展示。", [
            ...
            renderInputField("emptyText", "无歌词文案", "留空则隐藏"),
          ]),
          // 封面设置
          ...
          renderSection("外观", "调整字体、颜色。", [
            ...
            renderInputField("fontFamily", "字体（留空使用默认）", "例: Microsoft YaHei"),
...
`,
  ...
  // 浮窗自身的 sync+watch 会在检测到尺寸变化后调用 positionToTaskbar 修正位置
  ...
  // 主动常驻轮询：每 1s 调 recoverWindow 重新声明置顶+显示。
  // 浮窗自带的 topmostTimer（每 3s setAlwaysOnTop）兜底 z-order。
  // 解锁时跳过 recoverWindow：用户正在拖拽，ctx.windows.show 可能重置位置。
  // 同时检查心跳：若心跳 >3s 未更新（进程可能挂了），调用完整 showWindow 重建。
